...
- [x] Confirm, but add DTO suffixes for the SWA_101 envelope types (e.g. `{Entity}Envelope`, `ErrorItem`)
...
- [x] `methodName_scenario_expectedResult` — matches existing tests  ← recommended
...
- [x] By technical layer, as it exists today — drop the three phantom packages from the docs  ← recommended
...
- [x] ~70% unit / ~20% integration / ~10% end-to-end  ← recommended
...
- [x] 80% line coverage, enforced by JaCoCo — build fails below it  ← recommended
...
- [x] Yes — failing test first, minimal code to pass, refactor; never weaken a test to force a pass  ← recommended
...
- [x] Unit tests for the service layer (Mockito)  ← recommended
- [x] `@WebMvcTest` slice tests for the controller
- [x] Integration tests against a real Postgres via Testcontainers
- [x] Contract tests (Pact) — the existing doc lists `pact-jvm-consumer-junit5`, but it is **not** in `pom.xml` today
- [x] Persian-message assertions — verify user-facing text is actually Persian

### Q8. SWA_101 conformance testing *(required — select one)*

- [x] A shared test fixture asserts the envelope shape (`resultData`/`message`/`errorList`) on every endpoint  ← recommended
...
- [x] Checkstyle — enforces the formatting rules in Section 4  ← recommended
- [x] Spotless — auto-formats, fails the build on unformatted code
...
- [x] Yes — any violation fails `mvn verify`  ← recommended
...
- [x] Confirm as written — 4 spaces, 120 chars, K&R  ← recommended
...
- [x] One approving review before merge  ← recommended
...
- [x] Trunk-based — short-lived feature branches merged to `main` daily  ← recommended
...
- [x] Conventional Commits (`feat:`, `fix:`, `docs:`, `refactor:`)  ← recommended
...
- [x] All tests pass (`mvn test`)  ← recommended
- [x] Quality gates green — coverage per Q5 and static analysis per Q9/Q10
- [x] OpenAPI spec updated in the same PR (SAW_102 §2)
- [x] An ADR written if the task made a significant architectural decision (SAW_102 §4)
- [x] Persian user-facing messages reviewed by a Persian speaker
...
- [x] Confirm the existing allow/deny list as written  ← recommended
...
- [x] Confirm all seven, and add: never log a full request/response body (SWA_101 §10 already mandates this — this vote makes it locally visible too)
...
- [x] Compress to the cap — keep the rules, cut the long code samples, since the code itself is the better example  ← recommended
...
- [x] Required on public APIs, service methods, and non-obvious business logic  ← recommended
...
- [x] English for all code comments and Javadoc; Persian only for user-facing messages  ← recommended
- [ ] Persian for both
- [ ] Author's choice
- [ ] [TBD — needs input]
...
- [x] Persian, per the SAW_102 template exactly  ← recommended
...
- [x] Generate from SpringDoc annotations, then export the YAML to `/documents/openapi/` in CI  ← recommended
